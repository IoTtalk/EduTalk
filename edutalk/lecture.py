...
from edutalk.models import Lecture, Template, LectureProject, MacAddress
...
#    time_stop = args.timestop
    ...
    if df == 'Alcohol' or df == 'Humidity' or df == 'UV':
        dim = 1
    query_str = '{"_source":["sample","timestamp","to"],"query": {"bool": {"must": [{ "match": { "to": "' + df + '"}}],"filter":[{"range": {"timestamp": {"gte": "' +\
                time_start + '" ,"lte":"' + time_stop + '"}}}]}}}'
...
def delete(id_):
    ...
    for x in lecture.lecture_projects:
        x.delete()

    # delete input/output device model
    devicemodel.delete(lecture.idm)
    devicemodel.delete(lecture.odm)
    ...
    for j in lecture.joins:
        for df in j:
            if type(df) == str and df.startswith(lecture.odm + '_'):
